...
```
This program is unesisarily slow and contains may redundant expressions. The repetitive expressions such as `(uv - vec2(0.5, 0.5)) * 10` can have their value saved via a variable assignment. 
```c#
void program(inout vec3 color) { 
	float _cache_0x1 = sin(u_time);
